...
format_envdata <- function(in_env_dt) {
  ...
  in_env_dt[caudal %in% c(NA, 'SECO'), caudal := 0] %>% #Convert caudal/discharge to numeric
    ...
    .[, #Fill NAs by site for altura, pendiente, orden (height, slope, and stream order) 
      ...
      by='sitio'] %>%
    .[estado_de_flujo == 'seco', c('AH_med', 'prof_med', 'veloc_med') := 0] 
  
  in_env_dt[, cross_section_area := AH_med*prof_med] %>%
    .[, relative_cross_section_area := cross_section_area/max(cross_section_area, na.rm=T), 
      by=sitio]
...
fix_net_dangles <- function(in_net, in_tolerance, IDcol='OBJECTID_1') {
  ...
  for (i in 1:nrow(net_sub[net_sub[[IDcol]] %in% snapped_extremities[[IDcol]][,1], ])
       ) {
    line <- net_sub[net_sub[[IDcol]] %in% snapped_extremities[[IDcol]][,1], ][i,]
    #print(line[[IDcol]])

    new_node <- snapped_extremities[snapped_extremities[[IDcol]][,1] == line[[IDcol]], ]
...
compute_netdist_allsites <- function(in_sites_snapped, 
                                     in_net,
                                     IDcol) {
  #Import sites
...
plot_spdata <- function(in_spenv_dt, in_sp_dt) {
  ...
  plot_relative_abundance_vs_relative_area <- ggplot(in_spenv_dt, 
...
plot_envdata <- function(in_env_dt) {
  ...
  env_boxplots <- ggplot(melt(in_env_dt, 
                              id.vars=c('cuenca', 'presencia', 'sitio', 'fecha', 'intermitencia',
                                        'estado_de_flujo')),
                         aes(x=sitio, y=value)) +
